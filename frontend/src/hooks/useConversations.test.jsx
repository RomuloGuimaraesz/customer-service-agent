...
describe('useConversations', () => {
  ...
  describe('data fetching', () => {
    it('should fetch conversations successfully', async () => {
      ...
      await act(async () => {
        await result.current.conversations.fetchConversations()
      })

      await waitFor(() => {
        expect(result.current.conversations.loading.conversations).toBe(false)
      }, { timeout: 2000 })

      expect(result.current.conversations.conversations).toEqual(mockConversations)
      expect(result.current.conversations.error.conversations).toBeNull()
      expect(result.current.conversations.lastUpdated.conversations).toBeInstanceOf(Date)
    })
    ...
    it('should handle errors and use mock data', async () => {
      ...
      expect(result.current.conversations.conversations).toEqual(MOCK_DATA.conversations)
      // WhatsAppConversationsContext clears error after loading mock data (unlike PedidosContext which preserves it)
      expect(result.current.conversations.error.conversations).toBeNull()
    })
  })
